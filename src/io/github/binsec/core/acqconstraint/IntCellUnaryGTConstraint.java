...
public class IntCellUnaryGTConstraint extends UnaryConstraint {
	...
	public IntCellUnaryGTConstraint(ACQ_CellVariable cell1, int constant) {
		super("GT_"+constant, cell1.getValue().id);
	...
	public IntCellUnaryGTConstraint(ACQ_CellVariable cell1, int constant, boolean negated) {
		this(cell1, constant);
		if (negated) {
			this.setName("LE_" + constant);
			this.negated = negated;
		}
	}
	
	public IntCellUnaryGTConstraint(ACQ_CellVariable cell1, int constant, int weight) {
		this(cell1, constant);
		this.weight = weight;
	}
	
	public IntCellUnaryGTConstraint(ACQ_CellVariable cell1, int constant, boolean negated, int weight) {
	...
	public ACQ_IConstraint getNegation() {
		return new IntCellUnaryGTConstraint(this.cell1, this.constant, !this.negated);
	}
	
	public ACQ_IConstraint getNegation(int weight) {
		return new IntCellUnaryGTConstraint(this.cell1, this.constant, !this.negated, weight);
	}
	...
	public String getNegName() {
		if (!negated) {
			return "GT_" + this.constant;
		}
		else {
			return "LE_" + this.constant;
		}
	}
	...
	public String toSmtlib() {
		...
		if (!negated) {
			if (this.cell1.getType() == CellType.INT) {
				return "(sgt " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			}
			else if (this.cell1.getType() == CellType.UINT) {
				return "(ugt " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			}
			else {
				assert false;
				return null;
			...
			if (this.cell1.getType() == CellType.INT) {
				return "(not (sgt " + val1 + " " + String.format("#x%08x", this.constant) + "))";
			}
			else if (this.cell1.getType() == CellType.UINT) {
				return "(not (ugt " + val1 + " " + String.format("#x%08x", this.constant) + "))";
			}
			else {
				assert false;
				return null;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		IntCellUnaryGTConstraint other = (IntCellUnaryGTConstraint) obj;
