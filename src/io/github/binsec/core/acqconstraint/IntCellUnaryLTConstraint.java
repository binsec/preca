	...
	public String toSmtlib() {
		...
		if (!negated) {
			if (this.cell1.getType() == CellType.INT) {
				return "(slt " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			}
			else if (this.cell1.getType() == CellType.UINT) {
				return "(ult " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			}
			else {
				assert false;
				return null;
			...
			if (this.cell1.getType() == CellType.INT) {
				return "(not (slt " + val1 + " " + String.format("#x%08x", this.constant) + "))";
			}
			else if (this.cell1.getType() == CellType.UINT) {
				return "(not (ult " + val1 + " " + String.format("#x%08x", this.constant) + "))";
			}
			else {
				assert false;
				return null;
